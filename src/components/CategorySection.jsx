...
export default function CategorySection({category, items, addToCart}){
    return(
        <div className="bg-sec w-full">
            <h2 id={category} className="text-3xl flex p-4 w-full text-start font-oddval txt-acc">
            ...
            <div className="grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 place-items-center gap-5 my-5">
                {items.map(item => (
                    <div key={item.id} className="xl:w-[470px] xl:h-[225px] md:w-[470px] md:h-[225px] w-[350px] h-[225px] p-2 bg-sec txt-det rounded-lg transition-all duration-200 hover:scale-105 border-card">
                        <a role="button" onClick={()=>document.getElementById(`my_modal_${item.id}`).showModal()}>
                            <div className="flex items-center justify-between my-1">
                                <div className="w-[250px] flex flex-col justify-between txt-det">
                            ...
                            <dialog key={item.id} id={`my_modal_${item.id}`} className="modal">
                                <div className="modal-box bg-modal txt-sec">
                                    ...
                                    <div className="my-4">
                                        ...
                                        <p className="text-lg txt-sec">{item.description}</p>
                                    </div>
                                    <div>
                                        <h2 className="text-xl font-semibold">Ingredienti</h2>
                                        <p className="text-lg txt-sec">{item.ingredients}</p>
                                    </div>
                                    <div className="my-4">
                                        <h2 className="text-xl font-semibold">Prezzo</h2>
                                        <p className="text-lg txt-sec">{item.price}€</p>
                                    </div>
